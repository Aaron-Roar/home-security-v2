...
void initHall() {
    int counter = 0;
    while(counter < sizeof(hall_effect_array)/sizeof(hall_effect_array[0])) {
        pinMode(hall_effect_array[counter], INPUT);
        pinMode(hall_effect_light_array[counter], OUTPUT);
        counter += 1;
    }
}
